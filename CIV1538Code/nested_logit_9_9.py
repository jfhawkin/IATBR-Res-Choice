...
# Everything seems to be working except nest. Try just transit and transit cycle.
...
beta_dict = collections.OrderedDict([('ASC_AD', 1.5136), ('TIME', -0.0306), ('COST', -0.0403),
...
def logit_prob(params, const):
    ...
    PUN_AC = df['AV2'] * np.exp(V_AC)
    PUN_CPR = df['AV5'] * np.exp(V_CPR)
    ...
    PUD = PUN_AD + PUN_AC + PUN_CPR + PUN_PR + PUN_Trans_Act + PUN_C + PUN_W
    ...
    PU_AC = PU_AC.fillna(1)
    PU_CPR = PU_CPR.fillna(1)
    PU_PR = PU_PR.fillna(1)
    PU_Trans_Act = PU_Trans_Act.fillna(1)
    PU_C = PU_C.fillna(1)
    PU_W = PU_W.fillna(1)

    return PU_AD, PU_AC, PL_PT, PU_CPR, PL_CR, PU_PR, PU_C, PU_W, PU_Trans_Act

# Calculate unconditional probabilities from conditional probabilities
def sum_prob(params, const):
    PU_AD, PU_AC, PL_PT, PU_CPR, PL_CR, PU_PR, PU_C, PU_W, PU_Trans_Act = \
    logit_prob(params, const)

    return PU_AD, PU_AC, PL_PT * PU_Trans_Act, PU_CPR, PL_CR * PU_Trans_Act, PU_PR, PU_C, PU_W
...
def nest_logit(params, const):
    ...
    PU_AC = PU_AC.replace(0, 1)
    PU_CPR = PU_CPR.replace(0, 1)
    ...
    llfun = LL_PT.sum() + LL_CR.sum()

    # Upper nest calculations

    LL_AD = (df['CHOICE'] == CH['AD']) * np.log(PU_AD)
    LL_AC = np.log(PU_AC) * (df['CHOICE'] == CH['AC'])
    LL_CPR = np.log(PU_CPR) * (df['CHOICE'] == CH['CPR'])
    ...
    llfun = llfun + LL_AD.sum() + LL_AC.sum() + LL_CPR.sum() + LL_PR.sum() + LL_Trans_Act.sum() + LL_C.sum() + LL_W.sum()
    return -1 * llfun
